...
public class ViewTaskListAction
    extends BaseAction {
  ...
  public ActionForward doPerform(ActionMapping mapping, ActionForm form,
    ...
    if (pageIndex == null || pageIndex.equals("")) {
      if (flag.equals("0")) {
        utilizes = utilDao.getAllTaskByUser(loginName, pwd, "0", 1);
      }
      else if (flag.equals("1")) {
        utilizes = utilDao.getAllTaskByUser(loginName, pwd, "1", 1);
      }
      else if (flag.equals("2")) {
        utilizes = utilDao.getAllTaskByUser(loginName, pwd, "2", 1);
      }

      if (queryFlag != null) {
        if (!queryFlag.equals("3")) {
          if (queryFlag.equals("0")) {
            utilizes = utilDao.getAllTaskByCondition(loginName, pwd, "0", 1,
                dateDeliverBeginTime, dateDeliverEndTime, dateApplyBeginTime,
                dateApplyEndTime, utilizer);
          }
          else if (queryFlag.equals("1")) {
            utilizes = utilDao.getAllTaskByCondition(loginName, pwd, "1", 1,
                dateDeliverBeginTime, dateDeliverEndTime, dateApplyBeginTime,
                dateApplyEndTime, utilizer);
          }
          else if (queryFlag.equals("2")) {
            utilizes = utilDao.getAllTaskByCondition(loginName, pwd, "2", 1,
                dateDeliverBeginTime, dateDeliverEndTime, dateApplyBeginTime,
                dateApplyEndTime, utilizer);
          }
        }
